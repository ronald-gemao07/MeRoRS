                        ...
                        $.when(fetchingUsers).done(function(users) {
                            ...
                            usersListView.on('itemview:user:edit', function(childView, model) {
                                require(['apps/users/edit/edit_view'], function(EditView) {
                                    ...
                                    view.on('form:submit', function(data) {
                                        data.firstName = data.firstName.replace(/(<([^>]+)>)/ig, '');
                                        data.lastName = data.lastName.replace(/(<([^>]+)>)/ig, '');
                                        var regExPattern = /[a-zA-Z]{1,20}\.[a-zA-Z]{1,20}@globalzeal\.net$/i;
                                        if (!data.firstName) {
                                            this.$('div.firstname-error-message').show();
                                        } else if (!data.lastName) {
                                            this.$('div.lastname-error-message').show();
                                        } else if (!regExPattern.test(data.email)) {
                                            this.$('div.email-error-message').show();
                                        } else {
                                            if (model.save(data)) {
                                                childView.render();
                                                view.trigger('dialog:close');
                            ...
                            usersListView.on('itemview:user:delete', function(childView, model) {
                                //model.destroy();
                                require(['apps/users/delete/delete_view'], function(DeleteView) {
                                    var view = new DeleteView.User({
                                        model: model
                                    });

                                    view.on('user : delete', function(data) {
                                        model.destroy({
                                            wait: false
                                        });
                                        view.trigger('dialog:close');
